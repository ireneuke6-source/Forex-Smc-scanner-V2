"""
Deriv Forex SMC+ICT Scanner — OB + FVG Confluence Model (v2 — loosened)
Pairs: GBPUSD, EURUSD, XAUUSD, USDJPY, GBPJPY
Runs 24/7 on Railway.app

CHANGES FROM v1 (based on live log analysis showing signals were
extremely rare — roughly one per week across 5 pairs):

1. OB_MAX_AGE increased from 20 -> 35 H1 bars. Order blocks were
   expiring before a qualifying FVG had a chance to form near them.

2. FVG-to-OB relationship loosened from "must physically overlap"
   to "must be within proximity of the OB zone" (FVG_OB_PROXIMITY_PCT).
   This was the single biggest bottleneck observed live: EURUSD held
   a valid unmitigated OB for 15+ consecutive H1 bars without ever
   finding an FVG that literally overlapped it, even though gaps were
   forming nearby. Requiring literal overlap was too strict for how
   FVGs and OBs actually relate spatially in real price action.

3. Added per-layer rejection counters (_layer_stats) that log a daily
   summary showing exactly what % of scans died at each of the 4
   layers, per pair. This replaces guesswork with actual data so any
   future tuning is based on evidence, not assumption.

Entry Logic (all must align):
  H4  — Trend bias (EMA 21/50) + ADX strength
  H1  — Unmitigated Order Block, fresh within OB_MAX_AGE bars
...
COOLDOWN_FILE  = "cooldown_v2.json"
STATS_FILE     = "layer_stats_v2.json"
...
OB_LOOKBACK  = 30
OB_MIN_BODY  = 0.4
OB_MAX_AGE   = 35    # was 20 — widened so OBs survive long enough for an FVG to form nearby
...
def build_alert(
    ...
    fmt = f"%.{decimals}f"
    return (
        f"{icon}  &#8212;  <b>OB + FVG Confluence [V2 LOOSE]</b>\n"
        f"<code>{div}</code>\n"
...
async def main() -> None:
    ...
    log.info("=" * 60)
    log.info("Forex SMC+ICT OB+FVG Scanner v2 (loosened)  |  Started %s", start_time)
    log.info("OB_MAX_AGE=%d  FVG_OB_PROXIMITY_PCT=%.2f", OB_MAX_AGE, FVG_OB_PROXIMITY_PCT)
    log.info("=" * 60)
    ...
    send_telegram(
        f"🚀 <b>Forex SMC+ICT OB+FVG Scanner v2 LIVE</b>\n"
        f"<b>Pairs:</b> {', '.join(active_map.keys())}\n"
        f"<b>Model:</b> H4 Trend → H1 OB → M15 FVG (proximity) → M5 Entry\n"
        f"<b>Started:</b> {start_time}\n"
        f"<i>Loosened thresholds: OB age 35 bars, FVG proximity 0.15%.</i>\n"
